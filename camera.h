...
public:
    ...
    float Zoom;
    // camera perspective
    //glm::mat4 orthographicPerspective;
    ...
    // Process mouse scroll inputs for movement speed
    void ProcessMouseScroll(float yoffset)
    {
        if (yoffset > 0)         // Scroll up = faster movement
            MovementSpeed += 0.5f;
        if (yoffset < 0)         // Scroll down = slower movement
            MovementSpeed -= 0.5f;

        if (MovementSpeed < 0)        // Minimum speed
            MovementSpeed = 0.1f;
        if (MovementSpeed > 100.0f)   // Maximum speed
